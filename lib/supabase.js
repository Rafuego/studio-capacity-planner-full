...
export async function saveFullData(data) {
  ...
  if (data.team) {
    ...
    if (teamRecords.length > 0) {
      ...
      if (teamErr) console.error('Batch team upsert error:', teamErr);
    }
  }
  ...
  if (data.clients) {
    ...
    if (clientRecords.length > 0) {
      ...
      if (clientErr) console.error('Batch client upsert error:', clientErr);
    }
  }
  ...
  if (data.projects) {
    ...
    // Pre-fetch valid client IDs
    const { data: validClients } = await supabase.from('clients').select('id');
    const validClientIds = validClients ? validClients.map(c => c.id) : [];
    ...
    // Now batch save all relations - this is the expensive part
    // Collect ALL relation data, delete in bulk, insert in bulk
    const projectIds = data.projects.filter(p => p.id).map(p => p.id);

    if (projectIds.length > 0) {
      // Bulk delete all existing relations for these projects
      await Promise.all([
      ...
      // Batch insert all relations in parallel
      const insertPromises = [];
      if (allPhases.length > 0) insertPromises.push(
        supabase.from('project_phases').insert(allPhases).then(({ error }) => {
          if (error) console.error('Batch phases insert error:', error);
        })
      );
      if (allTeam.length > 0) insertPromises.push(
        supabase.from('project_team').insert(allTeam).then(({ error }) => {
          if (error) console.error('Batch team insert error:', error);
        })
      );
      if (allInvoices.length > 0) insertPromises.push(
        supabase.from('project_invoices').insert(allInvoices).then(({ error }) => {
          if (error) console.error('Batch invoices insert error:', error);
        })
      );
      if (allNotes.length > 0) insertPromises.push(
        supabase.from('project_notes').insert(allNotes).then(({ error }) => {
          if (error) console.error('Batch notes insert error:', error);
        })
      );
  ...
  if (data.projectTypes) {
    ...
    if (typeRecords.length > 0) {
      ...
      if (typeErr) console.error('Batch project types upsert error:', typeErr);
    }
  }
