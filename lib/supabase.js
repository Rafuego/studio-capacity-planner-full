...
export async function upsertProject(project) {
  ...
  if (project.team?.length > 0) {
    const teamAssignments = project.team.map(memberId => ({
      project_id: projectId,
      team_member_id: memberId
    }));
    
    await supabase.from('project_team').insert(teamAssignments);
  }
  ...
  if (project.budget?.invoices?.length > 0) {
    ...
    await supabase.from('project_invoices').insert(invoices);
  }
  ...
  if (project.notes?.length > 0) {
    ...
    await supabase.from('project_notes').insert(notes);
  }
