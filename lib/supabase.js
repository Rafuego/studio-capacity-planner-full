...
export async function saveFullData(data) {
  ...
  try {
    if (data.team) {
      ...
      // Split into existing (have id) and new (no id) records
      const existingTeam = data.team.filter(m => m.id).map(m => ({
        id: m.id,
        name: m.name,
        role: m.role,
      ...
      const newTeam = data.team.filter(m => !m.id).map(m => ({
        name: m.name,
        role: m.role,
        capacity: m.capacity || 40,
        rate: m.rate || 0,
      ...
      if (existingTeam.length > 0) {
        const { error: teamErr } = await supabase.from('team_members').upsert(existingTeam, { onConflict: 'id' });
        if (teamErr) errors.push({ step: 'team_upsert', error: teamErr.message });
      }
      if (newTeam.length > 0) {
        const { data: insertedTeam, error: teamInsErr } = await supabase.from('team_members').insert(newTeam).select();
        if (teamInsErr) errors.push({ step: 'team_insert', error: teamInsErr.message });
        // Map new IDs back into data.team so subsequent saves know these records exist
        if (insertedTeam) {
          insertedTeam.forEach((inserted, idx) => {
            const original = data.team.find(m => !m.id && m.name === inserted.name);
            if (original) original.id = inserted.id;
          });
        }
      }
    }
  ...
  try {
    if (data.clients) {
      ...
      // Split into existing (have id) and new (no id) records
      const existingClients = data.clients.filter(c => c.id).map(c => ({
        id: c.id,
        name: c.name,
        contact: c.contact,
        email: c.email,
      ...
      const newClients = data.clients.filter(c => !c.id).map(c => ({
        name: c.name,
        contact: c.contact,
        email: c.email,
        phone: c.phone,
      ...
      if (existingClients.length > 0) {
        const { error: clientErr } = await supabase.from('clients').upsert(existingClients, { onConflict: 'id' });
        if (clientErr) errors.push({ step: 'clients_upsert', error: clientErr.message });
      }
      if (newClients.length > 0) {
        const { data: insertedClients, error: clientInsErr } = await supabase.from('clients').insert(newClients).select();
        if (clientInsErr) errors.push({ step: 'clients_insert', error: clientInsErr.message });
        if (insertedClients) {
          insertedClients.forEach((inserted) => {
            const original = data.clients.find(c => !c.id && c.name === inserted.name);
            if (original) original.id = inserted.id;
          });
        }
      }
  ...
  try {
    if (data.projects) {
      ...
      // Split into existing (have id → upsert) and new (no id → insert)
      const existingProjectRecords = projectRecords.filter(r => r.id);
      const newProjectRecords = projectRecords.filter(r => !r.id);

      if (existingProjectRecords.length > 0) {
        const { error: projErr } = await supabase.from('projects').upsert(existingProjectRecords, { onConflict: 'id' });
        if (projErr) {
          console.error('Batch project upsert error:', projErr);
          errors.push({ step: 'projects_batch_upsert', error: projErr.message });
        }
      }
      if (newProjectRecords.length > 0) {
        const { data: insertedProjects, error: projInsErr } = await supabase.from('projects').insert(newProjectRecords).select();
        if (projInsErr) {
          console.error('Batch project insert error:', projInsErr);
          errors.push({ step: 'projects_batch_insert', error: projInsErr.message });
        } else if (insertedProjects) {
          // Map inserted IDs back to the original project data so relations can be saved
          insertedProjects.forEach((inserted, idx) => {
            const original = data.projects.find(p => !p.id && p.name === inserted.name);
            if (original) original.id = inserted.id;
          });
        }
      }
