...
export async function saveFullData(data) {
    ...
    if (data.projects) {
      ...
      if (projectIds.length > 0) {
        ...
        const insertResults = await Promise.all([
          allPhases.length > 0
            ? supabase.from('project_phases').insert(allPhases).then(r => ({ table: 'phases', ...r }))
            : { table: 'phases', error: null },
          allTeam.length > 0
            ? supabase.from('project_team').insert(allTeam).then(r => ({ table: 'team', ...r }))
            : { table: 'team', error: null },
          allInvoices.length > 0
            ? supabase.from('project_invoices').insert(allInvoices).then(r => ({ table: 'invoices', ...r }))
            : { table: 'invoices', error: null },
          allNotes.length > 0
            ? supabase.from('project_notes').insert(allNotes).then(r => ({ table: 'notes', ...r }))
            : { table: 'notes', error: null }
        ]);

        for (const r of insertResults) {
          if (r.error) {
            console.error(`Batch ${r.table} insert error:`, r.error);
            errors.push({ step: `${r.table}_insert`, error: r.error.message });
          }
        }
      }
    }
  ...
  if (errors.length > 0) {
    console.error('saveFullData completed with errors:', errors);
  }
