...
export async function saveFullData(data) {
  ...
  if (data.projects) {
    ...
    // Pre-fetch valid client IDs to avoid FK constraint failures
    const { data: validClients } = await supabase.from('clients').select('id');
    const validClientIds = validClients ? validClients.map(c => c.id) : [];

    // Process each project individually - don't let one failure break all saves
    for (const project of data.projects) {
      try {
        await upsertProject(project, validClientIds);
      } catch (err) {
        console.error(`Failed to save project "${project.name}" (id: ${project.id}):`, err.message);
